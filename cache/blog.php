...
<head>
    ...
    <!-- site metas -->
    <title>BlogCMS - <?php echo $blog["name"] ?></title>
...
<div class="text-center mt-5 w-100" style="">
    ...
    <h1 class="clear-both" style="font-size:2.5rem;"><?php echo $blog["name"] ?></h1>

    <img style="width: 80%; border: 3px #00b5f0 solid; " src="public/static/contents/blog_images/<?php echo $blog['path_copertina'] ?: 'default.png' ?>" alt="#" class="mb-2 blog_copertina"/>

    <?php if (array_key_exists("id", $_SESSION) && $_SESSION["id"] == $blog['id_author']): ?>
    <div class="mt-2" style="">
        <a href="editblog?id=<?php echo $blog['id'] ?>"><button class="btn btn-secondary mr-2"><i class="fa fa-pencil"></i> EDIT BLOG</button></a>
        <a href="deleteblog?id=<?php echo $blog['id'] ?>" onclick="return confirm('Are you sure?');"><button class="btn btn-danger"><i class="fa fa-trash"></i> DELETE BLOG</button></a>
    </div>
    <?php endif ?>
    <br>
    <?php if (array_key_exists("id", $_SESSION) && in_array($_SESSION["id"], array($blog['id_author'], $blog["id_coauthor"]))): ?>
        <div style="width: 40%; text-align: center; margin:0 auto;">
            <a href="creapost?id_blog=<?php echo $blog['id'] ?>"><button type="button" class="btn btn-primary btn-lg btn-block"> + ADD POST</button></a>
        </div>
    <?php endif ?>
    ...
    <div style="text-align: center" class="margin_top_30">
        ...
        <a class="" href="?id=<?php echo $blog['id'] ?>&page=<?php echo $i ?>">
            <?php endif ?>
            <?php echo $i ?>
            <?php if ($actual_page != $i): ?>
        </a>
